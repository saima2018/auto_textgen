...
class AutoRegressiveDecoder(object):
    ...
    def random_sample(self, inputs, n, topk=None, topp=None):
        ...
        logger.debug('inputs_ids.shape:{}'.format(inputs.shape))
        ...
        for step in range(self.maxlen):
            ...
            if topk is not None:
                # 为了是生成的文章不终止，所以需要将序列终止符号[SEP]的概率设置为0
                probas[:, self.end_id] = -0.1
            ...
            if output_ids.shape == (1, 0):
                output_ids = np.repeat(output_ids, sample_ids.shape[0], axis=0)
            if step >1:
                last_sample_ids = output_ids[:, -1:].tolist()
                for tmp_index in range(len(last_sample_ids)):
                    # 判断上一次预测的和这一次预测的都在“，。？；”中，则重新采样
                    if last_sample_ids[tmp_index][0] in [7922, 409, 7941, 1937] and sample_ids[tmp_index][0] in [7922, 409, 7941, 1937]:
                        tmp_k_indice = k_indices[tmp_index].tolist()
                        for tmp_id in tmp_k_indice:
                            if tmp_id in [7922, 409, 7941, 1937]:
                                tmp_k_indice.remove(tmp_id)
                        sample_ids[tmp_index] = choice(tmp_k_indice)

            output_ids = np.concatenate([output_ids, sample_ids], 1)  # 更新输出
            # print('第{}轮总花费时间：{}'.format(step, time.time()-pre_start))
        # 如果还有未完成序列，直接放入结果
    ...
    def random_sample_1(self, inputs, n, topk=None, topp=None):
        """随机采样n个结果
        说明：非None的topk表示每一步只从概率最高的topk个中采样；而非None的topp
             表示每一步只从概率最高的且概率之和刚好达到topp的若干个token中采样。
        返回：n个解码序列组成的list。
        ...
        for step in range(self.maxlen):
            ...
            if topp is not None:
                ...
                probas = np.take_along_axis(probas, p_indices, axis=1)  # 排序概率
                cumsum_probas = np.cumsum(probas, axis=1)  # 累积概率
                flag = np.roll(cumsum_probas >= int(topp), 1, axis=1)  # 标记超过topp的部分
                flag[:, 0] = False  # 结合上面的np.roll，实现平移一位的效果
                probas[flag] = 0  # 后面的全部置零
                probas /= probas.sum(axis=1, keepdims=True)  # 重新归一化
            sample_func = lambda p: np.random.choice(len(p), p=p)  # 按概率采样函数
